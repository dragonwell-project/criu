...
long __export_restore_task(struct task_restore_args *args)
{
	...
	*args->breakpoint = rst_sigreturn;
	...
	if (ret) {
		...
		pr_debug("  .start_code = %" PRIx64 "\n", (unsigned long)prctl_map.start_code);
		pr_debug("  .end_code = %" PRIx64 "\n", (unsigned long)prctl_map.end_code);
		pr_debug("  .start_data = %" PRIx64 "\n", (unsigned long)prctl_map.start_data);
		pr_debug("  .end_data = %" PRIx64 "\n", (unsigned long)prctl_map.end_data);
		pr_debug("  .start_stack = %" PRIx64 "\n", (unsigned long)prctl_map.start_stack);
		pr_debug("  .start_brk = %" PRIx64 "\n", (unsigned long)prctl_map.start_brk);
		pr_debug("  .brk = %" PRIx64 "\n", (unsigned long)prctl_map.brk);
		pr_debug("  .arg_start = %" PRIx64 "\n", (unsigned long)prctl_map.arg_start);
		pr_debug("  .arg_end = %" PRIx64 "\n", (unsigned long)prctl_map.arg_end);
		pr_debug("  .env_start = %" PRIx64 "\n", (unsigned long)prctl_map.env_start);
		pr_debug("  .env_end = %" PRIx64 "\n", (unsigned long)prctl_map.env_end);
		pr_debug("  .auxv_size = %" PRIu32 "\n", prctl_map.auxv_size);
		for (i = 0; i < prctl_map.auxv_size / sizeof(uint64_t); i++)
			pr_debug("  .auxv[%d] = %" PRIx64 "\n", i, (unsigned long)prctl_map.auxv[i]);
